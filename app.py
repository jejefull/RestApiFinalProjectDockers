...
conf={"log_file_location":"C:\\Users\\jeremy\\Documents\\LOGFILE\\ebaypro.log","log_level":"DEBUG","server":"10.0.0.2","user":"sa","password":"yourStrong(!)Password","database":"FLIGHTS"}


def init_logger():
    logging.basicConfig(filename=r'\opt\TEST.log',level='DEBUG')
#    logging.basicConfig(filename=r'C:\Users\jeremy\Documents\LOGFILE\TEST.log',level='DEBUG')

def test_db_connection():
    try:
        logging.info(f'Testing connection to [{conf["server"]}] [{conf["database"]}]', 'INFO')
        conn = pymssql._mssql.connect(server=conf['server'], user=conf['user'], password=conf['password'],
                                    database=conf['database'])
    except Exception as e:
        tr = traceback.format_exc()
        logging.error(f'Failed to connecto to db [{conf["server"]}] [{conf["database"]}]', 'ERROR')
        logging.error(f'Failed to connecto to db {e}', 'ERROR')
        logging.error(f'Failed to connecto to db {tr}', 'ERROR')
...
#        with pymssql._mssql.connect(server=dataBaseAddress,user='sa', password=dataBasePassword) as conn:
            ...
            with pymssql._mssql.connect(server=conf['server'], user=conf['user'], password=conf['password'],database=conf['database']) as conn:
...
#                    print('callproc!!!!!!!!! '+f'{new_customer["real_id"]} {new_customer["full_name"]}')
                ...
                logging.info(f'/customers POST new customer {new_customer}', 'INFO')
                logging.debug(f'/customers POST new customer query {query}', 'DEBUG')
        ...
        tr = traceback.format_exc()
        logging.error(f'Failed to run [{query}] to db {tr}', 'ERROR')
        print('error: ',e)
...
def update_by_id(id):
    ...
    try:
        with pymssql._mssql.connect(server=conf['server'], user=conf['user'], password=conf['password'],database=conf['database']) as conn:
            ...
            logging.info(f'/user PUT new user {update_customer}', 'INFO')
...
def SignIn_clients():
    try:
            logging.info('/SignIn_clients POST', 'INFO')
            query = ''
            ...
            with pymssql._mssql.connect(server=conf['server'], user=conf['user'], password=conf['password'],database=conf['database']) as conn:
                ...
                logging.info(f'/customers POST new customer {check_customer}', 'INFO')
                logging.debug(f'/customers POST new customer query {query}', 'DEBUG')
                conn.execute_query(query)
                result = []
...
def post_all_tickets():
    try:
        ...
        if request.method == 'POST':
            logging.info('/tickets POST', 'INFO')
            query = ''
            ...
            with pymssql._mssql.connect(server=conf['server'], user=conf['user'], password=conf['password'],database=conf['database']) as conn:
                ...
                if int(result[0]['REMAINING SEAT'])>0:#only if there is remainig seat, write to dba the buying
                    ...
                    logging.info(f'/customers POST new customer {new_ticket}', 'INFO')
                    #write to dba the buying ticket

                    logging.debug(f'/customers POST new TICKET query {query}', 'DEBUG')
                    conn.execute_query(query)
